...
function saves()
	if writefileExploit() and jsonAttempts < 50 then
		if pcall(function() readfile("z7n_AI.json") end) then
			if readfile("z7n_AI.json") ~= nil then
				...
				if not success then
					...
					writefileCooldown("z7n_AI.json", defaults)
					wait()
					saves()
				end
			else
...
function updatesaves()
	if nosaves == false and writefileExploit() then
		...
		writefileCooldown("z7n_AI.json", HttpService:JSONEncode(update))
	end
end
